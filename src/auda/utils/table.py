...
class Table:
    def __init__(
        self, headers: List[str], bold_headers: bool = True, fmt: str = 'simple'
    ) -> None:
        """
        ...
        Attributes:
            ...
            _headers_length: An integer representing the number of headers.
        """
        ...
        self._headers_length = len(headers)

    def append_row(self, row: List[Any]) -> Self:
        """
    ...
    def __repr__(self) -> str:
        ...
        This function uses the `tabulate` library to format the table in a readable way.
        """
        headers = self.headers
        if self.bold_headers:
            headers = [f'\033[1m{header}\033[0m' for header in self.headers]

        return tabulate(self.table, headers=headers, tablefmt=self.fmt)
