...
from auda.models import DataColumnMetadata, TableMetadata
from auda.utils.pagination import Pagination
...
class TableQueryParams(BaseModel):
    ...
    Args:
        column_names: List of column names to retrieve; if None, all columns are
            retrieved.
        sort_entries: List of SortEntry objects specifying how to sort the results.
        notnull_column_names: List of column names to filter out null values; if None,
            no filtering is applied.
        pagination: Pagination object specifying the page and page size for the
            results.
        aggregate_entry: AggregateEntry object specifying how to group and aggregate
            the results; if None, no aggregation is applied.
    """
...
class TableResult(BaseModel):
    ...
    Args:
        ...
        data: List of records, where each record is a list of values corresponding to
            the columns.
        pagination: Pagination object specifying the page and page size for the
            results.
...
class TableService:
    def __init__(self, database_name: DatabaseName):
        self.db = project.database_manager.get(database_name)
    ...
    def get_ui_column_map(self, table_name: str) -> Dict[str, str]:
        """
        Retrieves a mapping of column names to their UI column names for a specific data
        table.
        ...
        """
        table_metadata: Optional[TableMetadata] = self.get_table_metadata(table_name)
        ...
        if table_metadata.type == 'data':
            ...
            # The table is not a data table, then we return a mapping where the column
            # name is the same as the UI column name
            metadata = MetaData()
            table = Table(table_name, metadata, autoload_with=self.db.engine)
            column_names = [column.name for column in table.columns]
            ui_column_map = {column_name: column_name for column_name in column_names}
    ...
    def get_data_table_column_metadata(
        ...
        Args:
            table_name: The name of the data table to retrieve column metadata for.
    ...
    def prepare_tables(
        ...
        """
        Prepares the tables for querying by loading their metadata and SQLAlchemy Table
        objects.
        ...
        Returns:
            A tuple containing:
                - A list of SQLAlchemy Table objects for the specified table names.
    ...
    def get_column_map(self, tables: List[Table]) -> Dict[str, Column]:
        """
        Generates a mapping of column names to SQLAlchemy Column objects for the
        provided list of tables.
        ...
        """
        return {f'{tbl.name}.{col.name}': col for tbl in tables for col in tbl.columns}
    ...
    def get_record_count(
        ...
        """
        Retrieves the total number of records in the specified tables that match the
    ...
    def get_table_result(
        ...
        sort_entries: Optional[List[SortEntry]] = table_query_params.sort_entries
        ...
        aggregate_entry: Optional[AggregateEntry] = table_query_params.aggregate_entry

        # If the `column_names` is not an empty list, then use its shallow copy as the
        # `selected_column_names` list; otherwise, use the shallow copy of the
        # `all_column_names` list
        selected_column_names: List[str] = (
            column_names[:] if column_names else all_column_names[:]
        )

        # Create a list of SQLAlchemy columns based on the selected column names
        columns: List[Column] = []
        ...
        query = self.update_query_with_sort_entries(query, column_map, sort_entries)
        query = self.update_query_with_notnull_column_names(
        ...
        if (
            ...
            selected_column_names, records = self.postprocess_table_columns_records(
    ...
    def validate_table_metadata_map(
        ...
        Args:
            table_metadata_map: A dictionary mapping table names to their metadata.
        ...
        for table_name, metadata in table_metadata_map.items():
            if metadata.type != 'data':
                raise ValueError(
                    f"Table '{table_name}' is not a data table and cannot be joined.",
                )
    ...
    def build_column_name_list(self, tables: List[Table]) -> List[str]:
        """
        Builds a list of column names with table name aliasing to avoid ambiguity in
        SQL queries.
        ...
        Example:
            If `table1` has `col1` and `col2`, and `table2` has `col3` and `col4`,
            the `all_column_names` list will be:
    ...
    def update_query_with_notnull_column_names(
        ...
        for column_name in real_notnull_column_names:
            ...
            query = query.filter(and_(column.is_not(None), column != float('nan')))
    ...
    def get_aggregate_columns_and_functions(
        ...
        for group_by_column_name in group_by_column_names:
            ...
            # location)
            if group_by_column_name != 'year' and group_by_column_name != 'location':
                raise ValueError('Group by column names must be year or location.')
