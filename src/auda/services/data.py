...
class DataService:
    def __init__(self):
        ...
        """
        self.logger: Logger = project.get_logger('PrismService')
    ...
    def get_data_column_metadata_map(self) -> Dict[str, DataColumnMetadata]:
        """
        Creates a mapping of original data column names to their corresponding metadata
        objects.

        Args:
            data_column_metadata: A list of DataColumnMetadata objects.
        """
        return {
    ...
    def get_data_table_map(self) -> Dict[str, Table]:
        """
        Creates a mapping of data table names to their corresponding SQLAlchemy Table
        objects.

        Returns:
            A dictionary mapping table names to SQLAlchemy Table objects.
        """
        with self.auda_db.get_session() as session:
            table_list = session.query(TableMetadata).all()

        return {
            table.name: Table(table.name, MetaData(), autoload_with=session.bind)
    ...
    def _map_data_type(self, data_type: str):
        """
        Map the string in data_column_metadata.data_type to a SQLAlchemy type.
    ...
    def gather_columns_by_table(
        ...
        """
        Returns a mapping: table_name -> list of (column_name, sqlatype, comment).
        """
        sorted_rows = sorted(rows, key=lambda r: getattr(r, 'id', 0))
        grouped: Dict[str, List[Tuple[str, object, str]]] = defaultdict(list)

        for row in sorted_rows:
            column_type = self._map_data_type(str(getattr(row, 'data_type', None)))
    ...
    def create_data_tables(self) -> None:
        """
        Creates data tables in the database based on the data column metadata.
        """
        columns_by_table = self.gather_columns_by_table(self.get_data_column_metadata())
        ...
        with self.auda_db.get_session() as session:
            ...
            for table_name, columns in columns_by_table.items():
                ...
                self.logger.info(
                    f"Creating table '{table_name}' with {len(sa_columns)} columns."
                )
                tbl.create(bind=engine, checkfirst=True)

    def drop_data_tables(self) -> None:
        """
        Drops data tables in the database based on the data column metadata.
        """
        columns_by_table = self.gather_columns_by_table(self.get_data_column_metadata())
