...
from auda.core import Database, project
from auda.core.database import DatabaseName
from auda.models import PrismDataExtraction, PrismDataPoint, PrismLocation
from auda.models.prism_document import PrismDocument
...
class PrismService:
    ...
    def get_data_point_map(self) -> Dict[str, PrismDataPoint]:
        """
        Returns a dictionary mapping data point IDs to PrismDataPoint objects.

        Returns:
            A dictionary mapping data point IDs to PrismDataPoint objects.
        """
        return {str(data_point.id): data_point for data_point in self.get_data_points()}

    def get_data_points_with_description_and_counts(self) -> List[Tuple[str, str, int]]:
        """
        Fetches data points (data columns) along with their description and counts (the
        number of data extraction records) from the PRISM database.
        ...
        with self.prism_db.get_session() as session:
            query = (
                session.query(
                    ...
                    func.count(PrismDataPoint.name).label('count_of_data_points'),
                )
    ...
    def get_all_documents(self) -> List[PrismDocument]:
        """
        Fetches all documents from the PRISM database that have an extraction status of
        'EXTRACTED'.
    ...
    def get_document_by_id(self, document_id: UUID) -> PrismDocument:
        ...
        with self.prism_db.get_session() as session:
            query = session.query(PrismDocument).filter(PrismDocument.id == document_id)
            result = query.first()
