...
@step(
    id='PL-GPR',
    description='Generates plots for Gaussian Process Regression (GPR) '
    'model results.',
    input_specs=[
        Spec.ON.optional(Spec.DATASET.name).desc('Dataset to plot against.'),
        Spec.MODEL.desc('Trained GPR model'),
        Spec.X_MEAN,
        Spec.X_STD,
...
class GaussianProcessRegressionPlotter(
    ...
    def run(
        ...
        self.verify_model(model, GaussianProcessRegressor)
