...
from auda.model import (
    DataColumnMetadata,
    PrismDataExtraction,
    PrismDataPoint,
    PrismLocation,
)
from auda.service.data_service import DataService
from auda.service.prism_service import PrismService
...
class DataPoint(BaseModel):
    ...
    Attributes:
        ...
        data_point_id: The ID of the data point type.
...
class MigrationOptions(BaseModel):
    ...
    Attributes:
        max_count: The maximum number of data points to migrate. A value of -1
            means no limit. Default is -1.
        batch_size: The number of data points to process in each batch. Default
            is 100
        is_validated: If True, only migrate data points that have been
            validated; default to True.
        ...
        minimal_completeness_score: The minimum completeness score a data point
            must have to be migrated. Default is 0.
    """
...
class MigrationService:
    def __init__(self) -> None:
        ...
        Attributes:
            logger: The logger for the MigrationService.
            prism_db: The PRISM database instance.
            auda_db: The AUDA database instance.
            prism_service: The PrismService instance.
            data_service: The DataService instance.
        """
        self.logger: Logger = auda.get_logger(self.__class__.__name__)
        ...
        self.prism_service: PrismService = auda.singleton.get(PrismService)
        self.data_service: DataService = auda.singleton.get(DataService)

    def migrate(self, options: MigrationOptions) -> None:
        """Migrates data points from the AEPW database to the ADA database based
        ...
        def update_query(query: Select) -> Select:
            """Updates the given SQLAlchemy query based on the migration
            options.

            The `validation_status` can be either 'VALIDATED' or 'INVALIDATED'.

            The `data_completeness_score` is an integer between 0 and 100.
            """
            ...
            if not options.allow_duplicates:
                query = query.where(
                    PrismDataExtraction.is_duplicated.is_(False)
                )
        ...
        self.logger.info(
            f'Found {num_records} data points in the PRISM database.'
        )
        ...
        # Collect data column types from the `data_points` table in the Prism
        # database
        ...
        self.logger.info(
            f'Collected {len(prism_data_point_map)} data column types.'
        )
        ...
        self.logger.info(
            f'Collected {len(data_column_metadata_map)} pieces of data '
            'column metadata'
        )
        ...
        data_table_map: Dict[str, Table] = (
            self.data_service.get_data_table_map()
        )
    ...
    def migrate_data_point(
        ...
        """Migrates a single data point from the AEPW database to the AUDA
        database.

        Args:
            data_point: The data point to migrate.
            prism_data_point_map: A mapping of data point IDs to PrismDataPoint
                objects.
            prism_location_map: A mapping of location IDs to PrismLocation
                objects.
            data_column_metadata_map: A mapping of original column names to
                DataColumnMetadata objects.
            data_table_map: A mapping of data table names to SQLAlchemy Table
                objects.
        """
        ...
        if data_point_id not in prism_data_point_map:
            self.logger.debug(
                f'Data point ID "{data_point_id}" not found in data column '
        ...
        if original_column_name not in data_column_metadata_map:
            self.logger.debug(
                f'Original column name `{original_column_name}` not found in '
                'data column metadata map; the data point will be skipped.'
            )
            return False
        ...
        location: str = (
            prism_location.location_name if prism_location else 'Unknown'
        )
        year: int = data_point.year
    ...
    def insert_or_update_data_point(
        ...
        with self.auda_db.get_session() as ada_db_session:
            with ada_db_session.begin():
                ...
                result = cast(CursorResult, ada_db_session.execute(upd_stmt))
    ...
    def _coerce_for_column(self, column_type, raw):
        """Coerce raw into the right Python type for the SQLAlchemy column
        type.
        """
        ...
        if isinstance(column_type, (Integer, BigInteger, SmallInteger)):
            if d != d.to_integral_value():
                raise ValueError(
                    f'Expected an integer for this column, got a non-integer: '
                    f'{raw!r}'
                )
            return int(d)
