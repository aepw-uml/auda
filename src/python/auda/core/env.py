...
class Env:
    ...
    This class reads predefined environment variable names (as annotated
    attributes) from a `.env` file using `python-dotenv` and sets them as
    instance attributes.  If any required variable is missing, it raises a
    ValueError with a helpful message.

    Attributes are grouped into categories such as server settings, database
    settings, JWT configuration, and email service configuration.
    """
    ...
    def __get(self, name: str) -> str:
        ...
        if value is None:
            raise ValueError(
                ...
                f'Please define it in the ${self.dotenv_path} file.'
            )
