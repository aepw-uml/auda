...
% Codex
Random forest~\cite{breiman2001random} is an ensemble learning method that trains many decision trees on bootstrap samples of the data and combines their predictions. At each split, the model considers only a random subset of features, which reduces correlation among trees and improves generalization compared with a single decision tree. In this study, random forest is used for feature-importance analysis rather than as a primary forecasting model. The resulting importance scores provide a target-aware ranking of predictors, but they should be interpreted as empirical associations rather than causal effects.
