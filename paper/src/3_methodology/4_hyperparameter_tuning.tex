...
\[
	E = \frac{\sigma}{\sqrt{K}},
\]
where \(\sigma\) is the standard deviation of the fold-wise performance metrics for that configuration. We then identify all configurations whose mean performance is within the selection threshold \(\mu^\star + c_{\mathrm{SE}} E\), where \(\mu^\star\) is the best mean performance and \(c_{\mathrm{SE}}\) is a coefficient. Finally, we select the simplest configuration among these candidates. This process follows the 1-SE rule, which favors the simplest model whose cross-validation error is statistically close to the minimum-error model~\cite{breiman2017classification}.

The purpose of the 1-SE rule is to reduce accidental overfitting caused by randomness in hyperparameter search. The standard-error tolerance coefficient \(c_{\mathrm{SE}}\) controls how much loss in validation performance is allowed when selecting a simpler model. The smaller \(c_{\mathrm{SE}}\) is, the narrower the range of candidate configurations becomes and the more candidate configurations are excluded.
