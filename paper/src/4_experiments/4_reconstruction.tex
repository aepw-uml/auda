...
Table~\ref{tab:reconstruction_united_states_trc} shows a slightly different pattern for the \texttt{YearTRC (US)} task. Theil--Sen regression gives the best overall result, with a WAPE of 1.17\%, while linear interpolation is very close at 1.19\%. Ridge regression and SVR are also competitive, both with WAPE near 1.28\%. Relatively, machine learning models have worse performance in general. These results indicate that the United States series is easier to reconstruct with smoother trend-based methods, and that the best model can vary across countries even when the target variable and reconstruction setup are similar.
